(function (global, factory) {
	...
	//-----------------------------------------SCENE--------------------------------------------
	var choiceEnabled = null, CONTINU = null, REALMAX = null, CLEANING = null, DISSOLVE = 0.0, BGDISSOLVE = 0.4, ENDING = null, soundBand = null, CANCEL = null;
	...
	Object.assign(Scene.prototype, {
		...
		add : function(charac, texts, bg, effec)
		{
			...
			if((typeof(charac) == "object" && charac.type == "character") && typeof(texts) == "string")
			{
				...
				else if((bg == null) || (!bg))
				{
					...
					{
						let bac;
						if(choiceEnabled == false || choiceEnabled == null)
						{
							bac = this.Nodes[this.Nodes.length-1].background;
						}
						else if(choiceEnabled == true)
						{
							bac = this.Nodes[CONTINU].background;
						}
		...
		addChoice : function(choice1, choice2, meth1, meth2)
		{
			...
			if(typeof(choice1) == "string" && typeof(choice2) == "string")
			{
				...
				{
					this.linkList.push({id: this.Nodes.length-1, first: meth1, second: meth2});
				}
			}
		},
		...
		Loop : function()
		{
			...
			{
				if(choiceEnabled == true && CLEANING == true)
				{	
					for(var t = MAX-1; t >= this.choiceStart; t--)
					{
			...
			{
				if((this.currentAudio!= null) && (this.currentAudio != this.soundtracks[Check(this.soundtracks, this.current)].audio))
				{
					this.currentAudio.stop();
					this.currentAudio.remove();
				}
					
				this.currentAudio = this.soundtracks[Check(this.soundtracks, this.current)].audio;
			...
			else if(this.Nodes[this.current].type == "blackscreen")
			{
				...
				if(this.Nodes[this.current].sound == false)
				{
					this.currentAudio.stop();
					this.currentAudio.remove();
				}
			...
			if(WINNER == 1)
			{
				...
				if(Check(this.linkList, this.current) != -1)
				{
					...
					{	
						choiceEnabled = true;
						CONTINU = this.current;
						
						this.choiceStart = this.Nodes.length;
						REALMAX = MAX;
						
						this.linkList[Check(this.linkList, this.current)].first(this);
						
						CURRENT = this.choiceStart;
		...
		start: function(Ark)
		{
			...
			var func = function(){
				...
				if((ENDING != null) && (ENDING.max == MAX) && (ENDING.max == (CURRENT+1)))
				{
					ENDING.func();
					click.stop();
					soundBand.stop();
		...
		end: function(method)
		{
			...
			{
				this.ending = {func: method(), max: this.Nodes.length};
				ENDING = {func: method(), max: this.Nodes.length};
				//cancelAnimationFrame(this.cancel);
			}
			else
			{
				this.ending = {func: window.close(), max: this.Nodes.length};
				ENDING = {func: window.close(), max: this.Nodes.length};
			}
		},
			...
			if(((code) || (code == 0)) && (typeof(code) == "string" || typeof(code) == "number"))
			{
				...
				if(this.setUpCode == null)
				{
					...
					if(JSON.parse(localStorage.getItem(code)))
					{
						...
						{
							game.push({id: setUpCode, value: this, storage: this.storage, image: "none"});
						}
						else
						{
				...
				else
				{
					if(JSON.parse(localStorage.getItem(code)))
					{
						...
						if(found == false)
						{
							if(this.Nodes[this.current].background == null)
							{
								ALL.push({id: this.setUpCode, value: this, storage: this.storage, image: "none"});
							}
							else
							{
		...
		addSaving: function(saved)
		{
			...
			this.back = saved.value.back;
			this.choiceStart = saved.value.choiceStart;
			this.clickAudio = saved.value.clickAudio;
			this.current = saved.value.current;
			this.currentAudio = saved.value.currentAudio;
			...
			this.storage = saved.value.storage;
